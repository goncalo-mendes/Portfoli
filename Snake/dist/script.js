...
class App extends React.Component {
  constructor(props) {
    ...
    this.state = {
      snake: [],
      food: [],
      // 0 = nao começou, 1 = progresso, 2= acabou
      status: 0,
      // keydowns para direções
      direction: 39 };
  ...
  checkIfAteFood(newSnake) {
    ...
    // Se o comprimento da snake se torna mais longo
    let newSnakeSegment;
    ...
    // se a snake esta so movendo y, para mudar 
  ...
  render() {
    ...
    if (this.state.status === 0) {
      ...
      React.createElement("button", { onClick: this.startGame }, "Cemeçar o Jogo!"));


    } else if (this.state.status === 2) {
      overlay =
      React.createElement("div", { className: "snake-app__overlay" },
      React.createElement("div", { className: "mb-1" }, React.createElement("b", null, "Morreu!")),
      React.createElement("div", { className: "mb-1" }, "Pontução: ", this.state.snake.length, " "),
      React.createElement("button", { onClick: this.startGame }, "Começar outro?"));
...
ReactDOM.render(React.createElement(App, { size: 550 }), document.getElementById("root"));